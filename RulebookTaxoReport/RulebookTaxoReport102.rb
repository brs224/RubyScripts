...
begin
	...
	#	 	 puts "skipIncluded = #{skipIncluded}"   #0 = create excluded table
	 ...
	 currentDateTime66 = Time.now.strftime("%d%m%Y%H_%M_%S")
	 puts "Start Date: #{currentDateTime66}  ......"
	 
	 puts "zipping files...."
	 
	 zipFileName = "RulebookTaxoReport_#{currentDateTime66}.zip"
	 create_zipFile("outputFiles/",zipFileName)
	 
	 send_email2 "barbara.switzer@thomsonreuters.com", "test", "blah blah blah", zipFileName
    	 
     puts "exiting after send_email2.."
